#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include <time.h>
#include <intrin.h>  // Visual Studio: for __rdtsc()

#define TEST_GROUPS 100     // 明文样本组数
#define TEST_ROUNDS 10000   // 每组加解密轮数

// SM4 S-box
static const uint8_t SBox[256] = {
	
};

// 固定参数 CK
static const uint32_t CK[32] = {
	0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
	0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
	0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
	0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
	0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
	0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
	0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
	0x10171e25,0x2c333a41,0x484f565d,0x646b7279
};

// 左循环移位
uint32_t rotate_left(uint32_t val, int bits) {
	return (val << bits) | (val >> (32 - bits));
}

// 非线性变换 τ
uint32_t tau(uint32_t x) {
	return (SBox[x >> 24] << 24) | (SBox[(x >> 16) & 0xff] << 16) |
		(SBox[(x >> 8) & 0xff] << 8) | SBox[x & 0xff];
}

// 线性变换 L
uint32_t L(uint32_t B) {
	return B ^ rotate_left(B, 2) ^ rotate_left(B, 10) ^ rotate_left(B, 18) ^ rotate_left(B, 24);
}

// 线性变换 L'
uint32_t L_prime(uint32_t B) {
	return B ^ rotate_left(B, 13) ^ rotate_left(B, 23);
}

// 密钥扩展：加密或解密（方向由 reverse 控制）
void key_expansion(const uint8_t* key, uint32_t rk[32], int reverse) {
	uint32_t MK[4], K[36];
	memcpy(MK, key, 16);

	K[0] = MK[0] ^ 0xa3b1bac6;
	K[1] = MK[1] ^ 0x56aa3350;
	K[2] = MK[2] ^ 0x677d9197;
	K[3] = MK[3] ^ 0xb27022dc;

	for (int i = 0; i < 32; ++i) {
		uint32_t temp = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i];
		temp = tau(temp);
		rk[i] = K[i] ^ L_prime(temp);
		K[i + 4] = rk[i];
	}

	if (reverse) {
		for (int i = 0; i < 16; ++i) {
			uint32_t t = rk[i];
			rk[i] = rk[31 - i];
			rk[31 - i] = t;
		}
	}
}

// 每4轮展开
#define SM4_ROUND_4(i) do { \
    temp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]; \
    X[i + 4] = X[i] ^ L(tau(temp)); \
    temp = X[i + 2] ^ X[i + 3] ^ X[i + 4] ^ rk[i + 1]; \
    X[i + 5] = X[i + 1] ^ L(tau(temp)); \
    temp = X[i + 3] ^ X[i + 4] ^ X[i + 5] ^ rk[i + 2]; \
    X[i + 6] = X[i + 2] ^ L(tau(temp)); \
    temp = X[i + 4] ^ X[i + 5] ^ X[i + 6] ^ rk[i + 3]; \
    X[i + 7] = X[i + 3] ^ L(tau(temp)); \
} while (0)

// SM4加密/解密核心函数
void sm4_cipher(const uint8_t input[16], uint8_t output[16], const uint32_t rk[32]) {
	uint32_t X[36], temp;
	memcpy(X, input, 16);

	SM4_ROUND_4(0);
	SM4_ROUND_4(4);
	SM4_ROUND_4(8);
	SM4_ROUND_4(12);
	SM4_ROUND_4(16);
	SM4_ROUND_4(20);
	SM4_ROUND_4(24);
	SM4_ROUND_4(28);

	uint32_t out[4] = { X[35], X[34], X[33], X[32] };
	memcpy(output, out, 16);
}

// 显示字节数组
void print_block(const char* label, const uint8_t* data, int len) {
	printf("%s:\n", label);
	for (int i = 0; i < len; ++i)
		printf("%02x ", data[i]);
	printf("\n\n");
}


// 生成随机明文
void generate_random_plaintexts(uint8_t blocks[TEST_GROUPS][16]) {
	srand((unsigned int)time(NULL));
	for (int i = 0; i < TEST_GROUPS; ++i) {
		for (int j = 0; j < 16; ++j) {
			blocks[i][j] = rand() & 0xFF;
		}
	}
}

int main() {
	uint8_t key[16] = { 0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef };
	uint8_t plaintexts[TEST_GROUPS][16];
	uint8_t ciphertexts[TEST_GROUPS][16];
	uint8_t decrypted[16];
	uint32_t rk[32];

	generate_random_plaintexts(plaintexts);

	// ===== 加密 =====
	key_expansion(key, rk, 0);
	unsigned __int64 start, end;

	_ReadWriteBarrier();
	start = __rdtsc();
	for (int i = 0; i < TEST_ROUNDS; ++i) {
		for (int j = 0; j < TEST_GROUPS; ++j) {
			sm4_cipher(plaintexts[j], ciphertexts[j], rk);
		}
	}
	end = __rdtsc();
	_ReadWriteBarrier();

	uint64_t total_encrypt_cycles = end - start;
	printf("加密总周期：%llu，总加密次数：%d，平均每次：%.2f cycles\n",
		total_encrypt_cycles, TEST_GROUPS * TEST_ROUNDS,
		(double)total_encrypt_cycles / (TEST_GROUPS * TEST_ROUNDS));

	// ===== 解密 =====
	key_expansion(key, rk, 1);
	_ReadWriteBarrier();
	start = __rdtsc();
	for (int i = 0; i < TEST_ROUNDS; ++i) {
		for (int j = 0; j < TEST_GROUPS; ++j) {
			sm4_cipher(ciphertexts[j], decrypted, rk);
		}
	}
	end = __rdtsc();
	_ReadWriteBarrier();

	uint64_t total_decrypt_cycles = end - start;
	printf("解密总周期：%llu，总解密次数：%d，平均每次：%.2f cycles\n",
		total_decrypt_cycles, TEST_GROUPS * TEST_ROUNDS,
		(double)total_decrypt_cycles / (TEST_GROUPS * TEST_ROUNDS));

	return 0;
}

