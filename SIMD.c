...
void sm4_cipher_4x(const uint8_t input[4][16], uint8_t output[4][16], const uint32_t rk[32]) {
    uint32_t X[4][36];
    for (int b = 0; b < 4; ++b)
...
void sm4_decrypt_4x(const uint8_t input[4][16], uint8_t output[4][16], const uint32_t rk[32]) {
    uint32_t X[4][36];
    for (int b = 0; b < 4; ++b)
        memcpy(X[b], input[b], 16);
...
int main() {
    ...
    // ===== 加密 =====
    key_expansion(key, rk, 0);
    unsigned __int64 start, end;
    _ReadWriteBarrier();
    start = __rdtsc();
    for (int i = 0; i < TEST_ROUNDS; ++i) {
        for (int j = 0; j < TEST_GROUPS; j += 4) {
            sm4_cipher_4x(&plaintexts[j], &ciphertexts[j], rk);
        }
    }
    end = __rdtsc();
    _ReadWriteBarrier();
    ...
    // ===== 解密 =====
    key_expansion(key, rk, 1); 
    start = __rdtsc();
    for (int i = 0; i < TEST_ROUNDS; ++i) {
        for (int j = 0; j < TEST_GROUPS; j += 4) {
            sm4_decrypt_4x(&ciphertexts[j], &decrypted, rk);
        }
    }
    end = __rdtsc();
    _ReadWriteBarrier();
    uint64_t total_decrypt_cycles = end - start;
    printf("\n解密总周期：%llu，总解密次数：%d，平均每次解密：%.2f cycles\n",
